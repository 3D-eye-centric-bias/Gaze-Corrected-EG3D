...
def merge_images(img1, img2):
    ...
    img2_pil = to_pil_image(img2)

    width, height = img1_pil.size
    merged_img = PIL.Image.new('RGB', (2 * width, height))
    merged_img.paste(img1_pil, (0, 0))
    merged_img.paste(img2_pil, (width, 0))
...
def generate_images(
    network_pkl: str,
    network_pkl2: str, # Our Implementation
    seeds: List[int],
...
#--------------------------------Our Implementation------------------------------#
            ...
            img1, gaze_loss1, gaze_pitch1, gaze_yaw1 = generate_and_estimate(G1, z, camera_params, conditioning_params, truncation_psi, truncation_cutoff, gaze_estimator, device, face_pitch, face_yaw)
            img2, gaze_loss2, gaze_pitch2, gaze_yaw2 = generate_and_estimate(G2, z, camera_params, conditioning_params, truncation_psi, truncation_cutoff, gaze_estimator, device, face_pitch, face_yaw)
            
            # Accumulate gaze losses
            total_gaze_loss1 += gaze_loss1
            total_gaze_loss2 += gaze_loss2

            merged_img = merge_images(img1, img2)
            merged_img.save(f'{outdir}/seed{seed:04d}_compare_{i}.png')

            gaze_pitch1 = gaze_pitch1.cpu().item()
            gaze_yaw1 = gaze_yaw1.cpu().item()

            gaze_pitch2 = gaze_pitch2.cpu().item()
            gaze_yaw2 = gaze_yaw2.cpu().item()
    ...
    avg_gaze_loss1 = total_gaze_loss1 / num_seeds
    avg_gaze_loss2 = total_gaze_loss2 / num_seeds

    # Print the average gaze losses
    print(f'Average GFAS for Network 1: {avg_gaze_loss1}')
    print(f'Average GFAS for Network 2: {avg_gaze_loss2}')
#--------------------------------------------------------------------------------#
