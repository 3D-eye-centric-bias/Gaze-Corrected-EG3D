...
command = f"python 3dface2idr_mat.py --in_root Deep3DFaceRecon_pytorch/checkpoints/pretrained/results/{out_folder}/epoch_20_000000 --out_path {os.path.join(args.indir, 'cameras.json')}"
print(command)
os.system(command)

# additional correction to match the submission version
command = f"python preprocess_face_cameras.py --source {os.path.join(args.indir)} --dest {out_folder} --mode orig"
print(command)
os.system(command)